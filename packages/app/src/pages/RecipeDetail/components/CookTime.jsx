  ...
  adaptTime({ total, calcMins, calcHrs }) {
    try {
      if (total >= 0) {
        return {
          total,
          calcMins: Math.round(total % 60),
          calcHrs: Math.floor(total / 60),
        };
      }
      if (calcMins >= 0 && calcHrs >= 0) {
  ...
  render() {
    ...
    return (
      <TimeBox color={color} {...restProps}>
        ...
        {this.timeString(
          this.adaptTime({
            total: this.state.activeTotal + this.state.waitingTotal,
          }),
        )}
        ...
        {this.state.edit ? (
          <>
            <FieldWrapper label="hours" inline color={color}>
              <Input
                name="activeCalcHrs"
                value={this.state.activeCalcHrs}
            ...
            <FieldWrapper label="minutes" inline color={color}>
              <Input
                name="activeCalcMins"
                value={this.state.activeCalcMins}
                onChange={this.handleChange.bind(this)}
                inline
              />
            </FieldWrapper>
        ...
        {this.state.edit ? (
          <>
            <FieldWrapper label="hours" inline color={color}>
              <Input
                name="waitingCalcHrs"
                value={this.state.waitingCalcHrs}
                onChange={this.handleChange.bind(this)}
                inline
              />
            </FieldWrapper>
            <FieldWrapper label="minutes" inline color={color}>
              <Input
                name="waitingCalcMins"
                value={this.state.waitingCalcMins}
                onChange={this.handleChange.bind(this)}
                inline
              />
            </FieldWrapper>
